...
- [x] Google Analytics 연동 (2025.07.17)
...
### 2025.07.17 - Google Analytics 연동
#### 📊 분석 도구 추가
- **Google Analytics 4 설치**: 측정 ID `G-XKHSKJYP00` 연동
- **사용자 행동 추적**: 페이지뷰, 세션, 체류시간 자동 수집
- **성과 측정**: MBTI 테스트 완료율, 사용자 경로 분석 가능
- **데이터 기반 개선**: 사용자 데이터를 통한 서비스 최적화 기반 마련

#### 🔧 기술적 구현
- `_document.tsx`에 Google Analytics gtag 스크립트 추가
- `src/utils/gtag.ts` 추적 함수 유틸리티 생성
- **이벤트 추적 설정**:
  - 홈페이지: 테스트 시작 버튼 클릭 추적
  - 질문페이지: 각 질문별 답변 선택 추적 (질문 번호 + 답변 내용)
  - 결과페이지: MBTI 결과 확인 및 장소 추천 노출 추적
- **추적 가능한 데이터**:
  - 테스트 시작률 및 완료율
  - 질문별 응답 분포
  - MBTI 유형별 결과 분포
  - 추천 장소별 노출 빈도
- SEO와 함께 통합된 데이터 수집 환경 구축
